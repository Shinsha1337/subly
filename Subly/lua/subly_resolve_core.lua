...
local function hex_to_rgb(color)
    if type(color) == "table" then
        return tonumber(color[1]) or 255, tonumber(color[2]) or 255, tonumber(color[3]) or 255
    end
    local s = tostring(color or "")
    local r, g, b = s:match("^#?(%x%x)(%x%x)(%x%x)$")
    if not r then return nil end
    return tonumber(r, 16), tonumber(g, 16), tonumber(b, 16)
end

local function build_word_emphasis_styles(block, spans, base_size)
    local arr = {}
    for i, word in ipairs(block.words or {}) do
        local span = spans[i]
        if not span then break end
        local first = span.startIndex
        local last = span.endIndex

        if word.emphColor then
            local r, g, b = hex_to_rgb(word.emphColor)
            if r and g and b then
                table.insert(arr, { 2401, first, last, Value = r / 255, __flags = 256, Index = 0 })
                table.insert(arr, { 2402, first, last, Value = g / 255, __flags = 256, Index = 0 })
                table.insert(arr, { 2403, first, last, Value = b / 255, __flags = 256, Index = 0 })
            end
        end

        if word.emphSize then
            local pct = tonumber(word.emphSize)
            local size = tonumber(base_size)
            if pct and size then
                table.insert(arr, { 102, first, last, Value = size + (size * pct / 100), __flags = 256, Index = 0 })
            end
        end

        if word.emphFont and tostring(word.emphFont) ~= "" then
            local family = tostring(word.emphFont)
            local style = word.emphFontStyle and tostring(word.emphFontStyle) or ""
            if style == "" then
                -- Older builds stored registry labels such as "Bebas Neue : Black" as one font name.
                local legacy_family, legacy_style = family:match("^(.-)%s*:%s*(.-)$")
                if legacy_family and legacy_style and legacy_family ~= "" and legacy_style ~= "" then
                    family = legacy_family
                    style = legacy_style
                end
            end
            table.insert(arr, { 100, first, last, String = family, __flags = 256, Index = 0 })
            if style ~= "" then
                table.insert(arr, { 109, first, last, String = style, __flags = 256, Index = 0 })
            end
        end
    end
    return arr
...
local function apply_word_emphasis(comp, tool, block, text_value, pc_settings)
    ...
    -- to CharacterLevelStyling1.CharacterLevelStyling, not into the macro wrapper.
    local follower = nil
    pcall(function() follower = comp:FindTool("Follower1") end)
    if not follower then return false end

    local cls = nil
    pcall(function()
        local out = follower.Text:GetConnectedOutput()
        if out then cls = out:GetTool() end
    end)
    if not cls then
        pcall(function() cls = comp:FindTool("CharacterLevelStyling1") end)
    end
...
local function apply_autosubs_template(item, block, fps, pc_settings)
    ...
    pcall(function() comp:SetActiveTool(nil) end)
...
function SendFusionTextTitles(blocks, template_name, source_track, fill_gaps, max_frames, fallback_size, fallback_x, fallback_y)
    ...
    for i, item in ipairs(added) do
        pcall(function()
            if item.SetClipColor then item:SetClipColor("Purple") end
        end)
